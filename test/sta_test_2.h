...
class TestRunner2 : public CxxTest::TestSuite {
	Sta * sta;
...
public:
	...
	void test_min(){
		TS_ASSERT_EQUALS(stats["min"], -10);
	}
	void test_max(){
		TS_ASSERT_EQUALS(stats["max"], -1);
	}
	void test_mean(){
	...
	void test_q1(){
		TS_ASSERT_DELTA(stats["Q1"], -8, 1e-1);
	}
	void test_q3(){
		TS_ASSERT_DELTA(stats["Q3"], -3, 1e-1);
	}
	void test_percentiles(){
		...
		TS_ASSERT_EQUALS(stats["0th"], -10);
		TS_ASSERT_EQUALS(stats["44th"], -6);
		TS_ASSERT_EQUALS(stats["99th"], -1);
	}
	void test_sd_sample(){
		delete sta;
		opts["sample"] = 1;
